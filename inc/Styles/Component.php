...
use function WP_Rig\WP_Rig\kpbsd_codebase;
...
class Component implements Component_Interface, Templating_Component_Interface {
	...
	/**
	 * Gets template tags to expose as methods on the Template_Tags class instance, accessible through `kpbsd_codebase()`.
	 *
	...
	public function action_enqueue_styles() {
		...
		if ( ! empty( $google_fonts_url ) ) {
			wp_enqueue_style( 'kpbsd-codebase-fonts', $google_fonts_url, [], null ); // phpcs:ignore WordPress.WP.EnqueuedResourceParameters.MissingVersion
		}
		...
		foreach ( $css_files as $handle => $data ) {
			...
			$version = kpbsd_codebase()->get_asset_version( $css_dir . $data['file'] );
	...
	public function filter_resource_hints( array $urls, string $relation_type ) : array {
		if ( 'preconnect' === $relation_type && wp_style_is( 'kpbsd-codebase-fonts', 'queue' ) ) {
			$urls[] = [
		...
		$handles   = array_filter(
			...
			function( $handle ) use ( $css_files ) {
				...
				if ( ! $is_valid ) {
					...
					_doing_it_wrong( __CLASS__ . '::print_styles()', esc_html( sprintf( __( 'Invalid theme stylesheet handle: %s', 'kpbsd-codebase' ), $handle ) ), 'KPBSD CodeBase 2.0.0' );
				}
				return $is_valid;
			}
		);
	 ...
	 * {@see 'kpbsd_codebase_preloading_styles_enabled'} filter can be used to tweak the return value.
	 *
	...
	protected function preloading_styles_enabled() {
		$preloading_styles_enabled = ! kpbsd_codebase()->is_amp();
		 ...
		 */
		return apply_filters( 'kpbsd_codebase_preloading_styles_enabled', $preloading_styles_enabled );
	}
	...
	protected function get_css_files() : array {
		...
		$css_files = [
			'kpbsd-codebase-global'     => [
				'file'   => 'global.min.css',
				'global' => true,
			],
			'kpbsd-codebase-comments'   => [
				'file'             => 'comments.min.css',
				'preload_callback' => function() {
					return ! post_password_required() && is_singular() && ( comments_open() || get_comments_number() );
				},
			],
			'kpbsd-codebase-content'    => [
				'file'             => 'content.min.css',
				'preload_callback' => '__return_true',
			],
			'kpbsd-codebase-sidebar'    => [
				'file'             => 'sidebar.min.css',
				'preload_callback' => function() {
					return kpbsd_codebase()->is_primary_sidebar_active();
				},
			],
			'kpbsd-codebase-widgets'    => [
				'file'             => 'widgets.min.css',
				'preload_callback' => function() {
					return kpbsd_codebase()->is_primary_sidebar_active();
				},
			],
			'kpbsd-codebase-front-page' => [
			...
			'kpbsd-codebase-footer-widgets' => [
				'file' => 'footer-widgets.min.css',
				'preload_callback' => function() {
					return kpbsd_codebase()->is_footer_widgets_active();
				},
			],
		];
		 ...
		 */
		$css_files = apply_filters( 'kpbsd_codebase_css_files', $css_files );
	...
	protected function get_google_fonts() : array {
		 ...
		 */
		$this->google_fonts = (array) apply_filters( 'kpbsd_codebase_google_fonts', $google_fonts );
