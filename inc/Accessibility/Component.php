...
class Component implements Component_Interface {
	...
	public function action_enqueue_navigation_script() {
		...
		wp_enqueue_script(
			'kpbsd-codebase-navigation',
			get_theme_file_uri( '/assets/js/navigation.min.js' ),
			[],
			kpbsd_codebase()->get_asset_version( get_theme_file_path( '/assets/js/navigation.min.js' ) ),
			false
		);
		wp_script_add_data( 'kpbsd-codebase-navigation', 'async', true );
		wp_script_add_data( 'kpbsd-codebase-navigation', 'precache', true );
		wp_localize_script(
			'kpbsd-codebase-navigation',
			'kpbsdCodebaseScreenReaderText',
			[
				'expand'   => __( 'Expand child menu', 'kpbsd-codebase' ),
				'collapse' => __( 'Collapse child menu', 'kpbsd-codebase' ),
			]
		);
	}
